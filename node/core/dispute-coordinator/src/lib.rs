...
use polkadot_node_subsystem::{
    ...
    messages::{ChainApiMessage, DisputeCoordinatorMessage, DisputeParticipationMessage},
    ...
    loop {
        match ctx.recv().await? {
            ...
            FromOverseer::Communication { msg } => {
                handle_incoming(ctx, &**store, &mut state, config, msg).await?
            }
        }
    }
}
...
async fn handle_new_activations(
    ...
    for new_leaf in new_activations {
        let block_header = {
            ...
            ctx.send_message(ChainApiMessage::BlockHeader(new_leaf, tx).into())
                .await;
            ...
            .await
        {
            ...
            | Ok(SessionWindowUpdate::Advanced {
                ...
                if state.highest_session.map_or(true, |s| s < session) {
                    ...
                    db::v1::note_current_session(store, &config.column_config(), session)?;
                }
            }
            _ => {}
        }
...
async fn handle_incoming(
    ...
    match message {
        ...
        DisputeCoordinatorMessage::QueryCandidateVotes(session, candidate_hash, rx) => {
        ...
        DisputeCoordinatorMessage::DetermineUndisputedChain {
            ...
            let undisputed_chain =
                determine_undisputed_chain(store, &config, base_number, block_descriptions)?;
...
async fn handle_import_statements(
    ...
    if state
        .highest_session
        .map_or(true, |h| session + DISPUTE_WINDOW < h)
    {
        return Ok(());
    }
    ...
    let mut votes =
        db::v1::load_candidate_votes(store, &config.column_config(), session, &candidate_hash)?
    ...
    if freshly_disputed && !concluded_valid {
        ...
        update_active_disputes(store, config, &mut tx, |active| {
            active.insert(session, candidate_hash)
        })?;
    ...
    if concluded_valid && already_disputed {
        ...
        update_active_disputes(store, config, &mut tx, |active| {
            active.delete(session, candidate_hash)
        })?;
    }
...
fn update_active_disputes(
    ...
    let mut active_disputes =
        db::v1::load_active_disputes(store, &config.column_config())?.unwrap_or_default();
...
fn determine_undisputed_chain(
    ...
    for (i, (_, session, candidates)) in block_descriptions.iter().enumerate() {
        if candidates
            .iter()
            .any(|c| active_disputes.contains(*session, *c))
        {
            if i == 0 {
                return Ok(None);
            } else {
