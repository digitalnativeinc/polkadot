...
use sp_runtime::traits::{CheckedConversion, CheckedSub, Zero};
use sp_std::prelude::*;
...
impl<T: Config> crate::traits::OnSwap for Module<T> {
    fn on_swap(one: ParaId, other: ParaId) {
        Leases::<T>::mutate(one, |x| {
            Leases::<T>::mutate(other, |y| sp_std::mem::swap(x, y))
        })
    }
}
...
impl<T: Config> Leaser for Module<T> {
    ...
    fn lease_out(
        ...
        Leases::<T>::try_mutate(para, |d| {
            ...
            let period_count_usize = period_count
                .checked_into::<usize>()
                .ok_or(LeaseError::AlreadyEnded)?;
            ...
            Self::deposit_event(RawEvent::Leased(
                para,
                leaser.clone(),
                period_begin,
                period_count,
