...
use crate::traits::{Auctioneer, Registrar};
use frame_support::{
    ensure,
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        fn on_initialize(n: T::BlockNumber) -> frame_support::weights::Weight {
            if let Some(n) = T::Auctioneer::is_ending(n) {
                ...
                for (fund, para_id) in new_raise
                    .into_iter()
                    .filter_map(|i| Self::funds(i).map(|f| (f, i)))
                {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create(
            ...
            let last_period_limit = first_period
                ...
                last_period <= last_period_limit,
                Error::<T>::LastPeriodTooFarInFuture
            );
            ensure!(
            ...
            let manager = T::Registrar::manager_of(index).ok_or(Error::<T>::InvalidParaId)?;
                ...
                T::Registrar::is_registered(index),
                Error::<T>::InvalidParaId
            );
        ...
        pub fn contribute(
            ...
            let current_lease_period = T::Auctioneer::lease_period_index();
                ...
                current_lease_period <= fund.first_period,
                Error::<T>::ContributionPeriodOver
            );
        ...
        pub fn dissolve(origin: OriginFor<T>, #[pallet::compact] index: ParaId) -> DispatchResult {
            ...
            debug_assert!(Self::contribution_iterator(fund.trie_index)
                .count()
                .is_zero());
        ...
        pub fn add_memo(origin: OriginFor<T>, index: ParaId, memo: Vec<u8>) -> DispatchResult {
            let who = ensure_signed(origin)?;
                ...
                memo.len() <= T::MaxMemoLength::get().into(),
                Error::<T>::MemoTooLarge
            );
            let fund = Self::funds(index).ok_or(Error::<T>::InvalidParaId)?;
        ...
        pub fn poke(origin: OriginFor<T>, index: ParaId) -> DispatchResult {
            ...
            let fund = Self::funds(index).ok_or(Error::<T>::InvalidParaId)?;
                ...
                !NewRaise::<T>::get().contains(&index),
                Error::<T>::AlreadyInNewRaise
            );
            NewRaise::<T>::append(index);
...
impl<T: Config> crate::traits::OnSwap for Pallet<T> {
    fn on_swap(one: ParaId, other: ParaId) {
        Funds::<T>::mutate(one, |x| {
            Funds::<T>::mutate(other, |y| sp_std::mem::swap(x, y))
        })
    }
}
...
mod tests {
    ...
    use crate::{crowdloan, mock::TestRegistrar, traits::OnSwap};
    ...
    fn create_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                Crowdloan::create(Origin::signed(1), 1.into(), 1000, 1, 4, 9, None),
                e
            );
            // Cannot create a crowdloan with bad lease periods
            let e = Error::<Test>::LastPeriodBeforeFirstPeriod;
            assert_noop!(
                Crowdloan::create(Origin::signed(1), para, 1000, 4, 1, 9, None),
                e
            );
            let e = Error::<Test>::LastPeriodTooFarInFuture;
            assert_noop!(
                Crowdloan::create(Origin::signed(1), para, 1000, 1, 9, 9, None),
                e
            );
    ...
    fn multiple_refund_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                last_event(),
                super::Event::<Test>::PartiallyRefunded(para).into()
            );
    ...
    fn add_memo_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                Crowdloan::contribution_get(0u32, &1),
                (100, b"hello, world".to_vec())
            );
