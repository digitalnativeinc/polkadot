...
use crate::{configuration, dmp, hrmp, paras, scheduler::CoreAssignment, shared, ump};
...
impl<T: Config> Module<T> {
    ...
    pub(crate) fn process_bitfields(
        ...
        let signed_bitfields = {
            ...
            for unchecked_bitfield in unchecked_bitfields {
                    ...
                    last_index.map_or(true, |last| last
    ...
    pub(crate) fn process_candidates(
        ...
        let core_indices = core_indices_and_backers
            .iter()
...
mod tests {
    ...
    use crate::mock::{
        new_test_ext, Configuration, Inclusion, MockGenesisConfig, Paras, Shared, System, Test,
    };
    ...
    use primitives::v1::{
        CandidateCommitments, CandidateDescriptor, CollatorId, CompactStatement as Statement,
        SignedAvailabilityBitfield, SignedStatement, ValidationCode, ValidatorId,
        ValidityAttestation,
    };
    ...
    fn candidate_checks() {
        ...
        new_test_ext(genesis_config(paras)).execute_with(|| {
            ...
            // candidate not well-signed by collator.
            {
                ...
                assert_eq!(
                    CollatorId::from(Sr25519Keyring::Two.public()),
                    thread_collator
                );
    ...
    fn session_change_wipes() {
        ...
        new_test_ext(genesis_config(paras)).execute_with(|| {
            ...
            assert!(<AvailabilityBitfields<Test>>::iter()
                .collect::<Vec<_>>()
                .is_empty());
            assert!(<PendingAvailability<Test>>::iter()
                .collect::<Vec<_>>()
                .is_empty());
            assert!(<PendingAvailabilityCommitments>::iter()
                .collect::<Vec<_>>()
                .is_empty());
        });
    }
