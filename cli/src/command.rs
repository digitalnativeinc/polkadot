...
impl SubstrateCli for Cli {
    ...
    fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
        ...
        Ok(match id {
            ...
            name if name.starts_with("kusama-") => Err(format!(
                "`{}` only supported with `kusama-native` feature enabled.",
                name
            ))?,
            ...
            "rococo-opportunity" => Box::new(service::chain_spec::opportunity_config()?),
            // #[cfg(feature = "rococo-native")]
            // "rococo-opportunity" => Box::new(service::chain_spec::rococo_opportunity_testnet_config()?),
            #[cfg(feature = "rococo-native")]
            "rococo-opportunity-local" => {
                Box::new(service::chain_spec::rococo_opportunity_local_testnet_config()?)
            }
            #[cfg(feature = "rococo-native")]
            ...
            name if name.starts_with("rococo-") => Err(format!(
            ...
            name if name.starts_with("westend-") => Err(format!(
                "`{}` only supported with `westend-native` feature enabled.",
                name
            ))?,
            ...
            name if name.starts_with("wococo-") => Err(format!(
                "`{}` only supported with `rococo-native` feature enabled.",
                name
            ))?,
            path => {
