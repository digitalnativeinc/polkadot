...
fn read_write() {
    ...
    assert_eq!(
        load_stored_blocks(&store, &TEST_CONFIG).unwrap(),
        Some(range)
    );
    assert_eq!(
        load_blocks_at_height(&store, &TEST_CONFIG, 1).unwrap(),
        at_height
    );
    assert_eq!(
        load_block_entry(&store, &TEST_CONFIG, &hash_a).unwrap(),
        Some(block_entry)
    );
    assert_eq!(
    ...
    assert!(load_blocks_at_height(&store, &TEST_CONFIG, 1)
        .unwrap()
        .is_empty());
    assert!(load_block_entry(&store, &TEST_CONFIG, &hash_a)
        .unwrap()
        .is_none());
    assert!(load_candidate_entry(&store, &TEST_CONFIG, &candidate_hash)
        .unwrap()
        .is_none());
}
...
fn add_block_entry_works() {
    ...
    let block_entry_b = make_block_entry(
        ...
        block_number,
        vec![
            (CoreIndex(0), candidate_hash_a),
            (CoreIndex(1), candidate_hash_b),
        ],
    );
    ...
    assert_eq!(
        load_block_entry(&store, &TEST_CONFIG, &block_hash_a).unwrap(),
        Some(block_entry_a)
    );
    assert_eq!(
        load_block_entry(&store, &TEST_CONFIG, &block_hash_b).unwrap(),
        Some(block_entry_b)
    );
...
fn canonicalize_works() {
    ...
    for block_entry in blocks {
        add_block_entry(&store, &TEST_CONFIG, block_entry, n_validators, |h| {
    ...
    let check_candidates_in_store = |expected: Vec<(CandidateHash, Option<Vec<_>>)>| {
        for (c_hash, in_blocks) in expected {
            let (entry, in_blocks) = match in_blocks {
                None => {
                    assert!(load_candidate_entry(&store, &TEST_CONFIG, &c_hash)
                        .unwrap()
                        .is_none());
                    continue;
                }
                Some(i) => (
                    load_candidate_entry(&store, &TEST_CONFIG, &c_hash)
                        .unwrap()
                        .unwrap(),
                    i,
                ),
    ...
    let check_blocks_in_store = |expected: Vec<(Hash, Option<Vec<_>>)>| {
        for (hash, with_candidates) in expected {
            let (entry, with_candidates) = match with_candidates {
                None => {
                    assert!(load_block_entry(&store, &TEST_CONFIG, &hash)
                        .unwrap()
                        .is_none());
                    continue;
                }
                Some(i) => (
                    load_block_entry(&store, &TEST_CONFIG, &hash)
                        .unwrap()
                        .unwrap(),
                    i,
                ),
    ...
    assert_eq!(
        load_stored_blocks(&store, &TEST_CONFIG).unwrap().unwrap(),
...
fn force_approve_works() {
    ...
    assert!(load_block_entry(&store, &TEST_CONFIG, &block_hash_a,)
        .unwrap()
        .unwrap()
        .approved_bitfield
        .all());
    assert!(load_block_entry(&store, &TEST_CONFIG, &block_hash_b,)
        .unwrap()
        .unwrap()
        .approved_bitfield
        .all());
    assert!(load_block_entry(&store, &TEST_CONFIG, &block_hash_c,)
        .unwrap()
        .unwrap()
        .approved_bitfield
        .not_any());
    assert!(load_block_entry(&store, &TEST_CONFIG, &block_hash_d,)
        .unwrap()
        .unwrap()
        .approved_bitfield
        .not_any());
...
fn load_all_blocks_works() {
    ...
    assert_eq!(
        load_all_blocks(&store, &TEST_CONFIG).unwrap(),
