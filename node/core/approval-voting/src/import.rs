...
async fn imported_block_info(
    ...
    let force_approve =
        block_header
            .digest
            .convert_first(|l| match ConsensusLog::from_digest_item(l) {
...
pub(crate) async fn handle_new_head(
    ...
    match state
        .session_window
        .cache_session_info_for_head(ctx, head, &header)
        .await
    {
        Err(e) => {
            tracing::debug!(
    ...
    for (block_hash, block_header, imported_block_info) in imported_blocks_and_info {
        ...
        let validator_group_lens: Vec<usize> = session_info
            .validator_groups
            .iter()
            .map(|v| v.len())
            .collect();
        ...
        let approved_bitfield = {
            if needed_approvals == 0 {
                ...
                for (i, &(_, _, _, backing_group)) in included_candidates.iter().enumerate() {
                    let backing_group_size = validator_group_lens
                        .get(backing_group.0 as usize)
                        .copied()
                        .unwrap_or(0);
...
mod tests {
    ...
    use crate::{criteria, BlockEntry, APPROVAL_SESSIONS};
    ...
    fn single_session_state(index: SessionIndex, info: SessionInfo) -> State<TestDB> {
        State {
        ...
        let test_fut = {
            Box::pin(async move {
                ...
                let entry: BlockEntry =
                    crate::approval_db::v1::load_block_entry(&db_writer, &TEST_CONFIG, &hash)
                        .unwrap()
                        .unwrap()
                        .into();
