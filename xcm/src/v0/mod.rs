...
pub mod prelude {
    pub use super::junction::{BodyId, BodyPart, Junction::*, NetworkId};
    pub use super::multi_asset::{
        AssetInstance::{self, *},
        MultiAsset::{self, *},
    };
    pub use super::multi_location::MultiLocation::{self, *};
    pub use super::order::Order::{self, *};
    pub use super::traits::{Error as XcmError, ExecuteXcm, Outcome, Result as XcmResult, SendXcm};
    pub use super::{
        OriginKind,
        Xcm::{self, *},
    };
...
impl<Call> Xcm<Call> {
    ...
    pub fn from<C>(xcm: Xcm<C>) -> Self {
        use Xcm::*;
        match xcm {
            WithdrawAsset { assets, effects } => WithdrawAsset {
                assets,
            ...
            TeleportAsset { assets, effects } => TeleportAsset {
                assets,
                effects: effects.into_iter().map(Order::into).collect(),
            },
            QueryResponse {
            ...
            RelayedFrom { who, message } => RelayedFrom {
                who,
                message: alloc::boxed::Box::new((*message).into()),
            },
        }
    }
}
