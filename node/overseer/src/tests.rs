...
use futures::{executor, pending, pin_mut, select, FutureExt};
...
use polkadot_subsystem::{jaeger, messages::NetworkBridgeEvent, messages::RuntimeApiRequest};
...
fn overseer_metrics_work() {
    ...
    executor::block_on(async move {
        ...
        handler
            .send_msg(AllMessages::CandidateValidation(
...
fn overseer_all_subsystems_receive_signals_and_messages() {
    ...
    executor::block_on(async move {
        ...
        handler
            ...
            .await;
        handler
            .send_msg(AllMessages::CandidateBacking(test_candidate_backing_msg()))
            .await;
        handler
            .send_msg(AllMessages::CollationGeneration(
                test_collator_generation_msg(),
            ))
            .await;
        handler
            .send_msg(AllMessages::CollatorProtocol(test_collator_protocol_msg()))
            .await;
        handler
            .send_msg(AllMessages::StatementDistribution(
                test_statement_distribution_msg(),
            ))
            .await;
        handler
            .send_msg(AllMessages::AvailabilityRecovery(
        ...
        handler
            .send_msg(AllMessages::BitfieldDistribution(
        ...
        handler
            ...
            .await;
        handler
            .send_msg(AllMessages::AvailabilityStore(test_availability_store_msg()))
            .await;
        handler
            .send_msg(AllMessages::NetworkBridge(test_network_bridge_msg()))
            .await;
        handler
            .send_msg(AllMessages::ChainApi(test_chain_api_msg()))
            .await;
        handler
            .send_msg(AllMessages::ApprovalDistribution(
                test_approval_distribution_msg(),
            ))
            .await;
        handler
            .send_msg(AllMessages::ApprovalVoting(test_approval_voting_msg()))
            .await;

        // Wait until all subsystems have received. Otherwise the messages might race against
        // the conclude signal.
        loop {
            match (&mut overseer_fut)
        ...
        assert_eq!(
            stop_signals_received.load(atomic::Ordering::SeqCst),
            NUM_SUBSYSTEMS
        );
        assert_eq!(
            signals_received.load(atomic::Ordering::SeqCst),
            NUM_SUBSYSTEMS
        );
        assert_eq!(
            msgs_received.load(atomic::Ordering::SeqCst),
            NUM_SUBSYSTEMS_MESSAGED
        );
