...
use polkadot_node_subsystem_util::{
    ...
    Metronome, TimeoutExt,
};
use polkadot_procmacro_overseer_subsystems_gen::AllSubsystemsGen;
use polkadot_subsystem::messages::{
    AllMessages, ApprovalDistributionMessage, ApprovalVotingMessage,
    ...
    CandidateValidationMessage, ChainApiMessage, CollationGenerationMessage,
    CollatorProtocolMessage, GossipSupportMessage, NetworkBridgeMessage, ProvisionerMessage,
    RuntimeApiMessage, StatementDistributionMessage,
};
pub use polkadot_subsystem::{
    jaeger, ActivatedLeaf, ActiveLeavesUpdate, DummySubsystem, FromOverseer, LeafStatus,
    OverseerSignal, SpawnedSubsystem, Subsystem, SubsystemContext, SubsystemError, SubsystemResult,
    SubsystemSender,
};
    ...
    Client::Api: ParachainHost<Block>,
{
    fn head_supports_parachains(&self, head: &Hash) -> bool {
        ...
        self.runtime_api()
            .has_api::<dyn ParachainHost<Block>>(&id)
            .unwrap_or(false)
    }
}
...
type AllSubsystemsSame<T> = AllSubsystems<T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T, T>;
...
impl OverseerHandler {
    ...
    pub async fn send_msg(&mut self, msg: impl Into<AllMessages>) {
        self.send_and_log_error(Event::MsgToSubsystem(msg.into()))
            .await
    }
...
struct ChannelsOut {
    ...
    availability_distribution:
        metered::MeteredSender<MessagePacket<AvailabilityDistributionMessage>>,
    ...
    candidate_validation_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<CandidateValidationMessage>>,
    candidate_backing_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<CandidateBackingMessage>>,
    statement_distribution_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<StatementDistributionMessage>>,
    availability_distribution_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<AvailabilityDistributionMessage>>,
    availability_recovery_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<AvailabilityRecoveryMessage>>,
    bitfield_signing_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<BitfieldSigningMessage>>,
    bitfield_distribution_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<BitfieldDistributionMessage>>,
    ...
    availability_store_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<AvailabilityStoreMessage>>,
    ...
    collation_generation_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<CollationGenerationMessage>>,
    collator_protocol_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<CollatorProtocolMessage>>,
    approval_distribution_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<ApprovalDistributionMessage>>,
    approval_voting_unbounded:
        metered::UnboundedMeteredSender<MessagePacket<ApprovalVotingMessage>>,
    gossip_support_unbounded: metered::UnboundedMeteredSender<MessagePacket<GossipSupportMessage>>,
}

impl ChannelsOut {
    async fn send_and_log_error(&mut self, signals_received: usize, message: AllMessages) {
        let res = match message {
            AllMessages::CandidateValidation(msg) => {
                self.candidate_validation
                    .send(make_packet(signals_received, msg))
                    .await
            }
    ...
    fn send_unbounded_and_log_error(&self, signals_received: usize, message: AllMessages) {
        let res = match message {
            AllMessages::CandidateValidation(msg) => self
                .candidate_validation_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::CandidateBacking(msg) => self
                .candidate_backing_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::StatementDistribution(msg) => self
                .statement_distribution_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::AvailabilityDistribution(msg) => self
                .availability_distribution_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::AvailabilityRecovery(msg) => self
                .availability_recovery_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::BitfieldDistribution(msg) => self
                .bitfield_distribution_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::BitfieldSigning(msg) => self
                .bitfield_signing_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::Provisioner(msg) => self
                .provisioner_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::RuntimeApi(msg) => self
                .runtime_api_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::AvailabilityStore(msg) => self
                .availability_store_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::NetworkBridge(msg) => self
                .network_bridge_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::ChainApi(msg) => self
                .chain_api_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::CollationGeneration(msg) => self
                .collation_generation_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::CollatorProtocol(msg) => self
                .collator_protocol_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::ApprovalDistribution(msg) => self
                .approval_distribution_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::ApprovalVoting(msg) => self
                .approval_voting_unbounded
                .unbounded_send(make_packet(signals_received, msg))
                .map_err(|e| e.into_send_error()),
            AllMessages::GossipSupport(msg) => self
...
impl SubsystemSender for OverseerSubsystemSender {
    async fn send_message(&mut self, msg: AllMessages) {
        self.channels
            .send_and_log_error(self.signals_received.load(), msg)
            .await;
    }
    ...
    fn send_unbounded_message(&mut self, msg: AllMessages) {
        self.channels
            .send_unbounded_and_log_error(self.signals_received.load(), msg);
    }
}
...
impl<M: Send + 'static> SubsystemContext for OverseerSubsystemContext<M> {
        ...
        loop {
            ...
            if let Some((needs_signals_received, msg)) = self.pending_incoming.take() {
                if needs_signals_received <= self.signals_received.load() {
                    ...
                    let signal = self.signals.next().await.ok_or(SubsystemError::Context(
    ...
    async fn spawn(
        ...
        self.to_overseer
            .send(ToOverseer::SpawnJob { name, s })
            .await
            .map_err(Into::into)
    }
    ...
    async fn spawn_blocking(
        ...
        self.to_overseer
            .send(ToOverseer::SpawnBlockingJob { name, s })
            .await
            .map_err(Into::into)
    }
    ...
    SupportsParachains: HeadSupportsParachains,
{
        ...
        GS: Subsystem<OverseerSubsystemContext<GossipSupportMessage>> + Send,
    {
        ...
        let (bitfield_distribution_bounded_tx, bitfield_distribution_bounded_rx) =
            metered::channel(CHANNEL_CAPACITY);
        let (provisioner_bounded_tx, provisioner_bounded_rx) = metered::channel(CHANNEL_CAPACITY);
        let (runtime_api_bounded_tx, runtime_api_bounded_rx) = metered::channel(CHANNEL_CAPACITY);
        let (availability_store_bounded_tx, availability_store_bounded_rx) =
            metered::channel(CHANNEL_CAPACITY);
        let (network_bridge_bounded_tx, network_bridge_bounded_rx) =
            metered::channel(CHANNEL_CAPACITY);
        let (chain_api_bounded_tx, chain_api_bounded_rx) = metered::channel(CHANNEL_CAPACITY);
        ...
        let (candidate_validation_unbounded_tx, candidate_validation_unbounded_rx) =
            metered::unbounded();
        let (candidate_backing_unbounded_tx, candidate_backing_unbounded_rx) = metered::unbounded();
        ...
        let (availability_recovery_unbounded_tx, availability_recovery_unbounded_rx) =
            metered::unbounded();
        let (bitfield_signing_unbounded_tx, bitfield_signing_unbounded_rx) = metered::unbounded();
        let (bitfield_distribution_unbounded_tx, bitfield_distribution_unbounded_rx) =
            metered::unbounded();
        let (provisioner_unbounded_tx, provisioner_unbounded_rx) = metered::unbounded();
        let (runtime_api_unbounded_tx, runtime_api_unbounded_rx) = metered::unbounded();
        let (availability_store_unbounded_tx, availability_store_unbounded_rx) =
            metered::unbounded();
        let (network_bridge_unbounded_tx, network_bridge_unbounded_rx) = metered::unbounded();
        let (chain_api_unbounded_tx, chain_api_unbounded_rx) = metered::unbounded();
        let (collator_protocol_unbounded_tx, collator_protocol_unbounded_rx) = metered::unbounded();
        ...
        let (approval_distribution_unbounded_tx, approval_distribution_unbounded_rx) =
            metered::unbounded();
        let (approval_voting_unbounded_tx, approval_voting_unbounded_rx) = metered::unbounded();
        let (gossip_support_unbounded_tx, gossip_support_unbounded_rx) = metered::unbounded();
        ...
        let statement_distribution_subsystem = spawn(
            ...
            stream::select(
                statement_distribution_bounded_rx,
        ...
        let availability_recovery_subsystem = spawn(
            ...
            stream::select(
                availability_recovery_bounded_rx,
        ...
        let availability_store_subsystem = spawn(
            ...
            stream::select(
                ...
                availability_store_unbounded_rx,
            ),
        ...
        let collation_generation_subsystem = spawn(
            ...
            stream::select(
                collation_generation_bounded_rx,
        ...
        let approval_distribution_subsystem = spawn(
            ...
            stream::select(
                approval_distribution_bounded_rx,
...

        {
            ...
            impl<'a, T: 'a> MapSubsystem<&'a OverseenSubsystem<T>> for ExtractNameAndMeters {
                ...
                fn map_subsystem(&self, subsystem: &'a OverseenSubsystem<T>) -> Self::Output {
                    let instance = subsystem.instance.as_ref().expect(
                        "Extraction is done directly after spawning when subsystems\
						have not concluded; qed",
                    );

                    (instance.name, instance.meters.clone())
                }
            }
            ...
            let metronome =
                Metronome::new(std::time::Duration::from_millis(950)).for_each(move |_| {
                    let subsystem_meters = subsystem_meters.as_ref().map_subsystems(
                        |&(name, ref meters): &(_, SubsystemMeters)| (name, meters.read()),
                    );
    ...
    pub async fn run(mut self) -> SubsystemResult<()> {
        ...
        if !update.is_empty() {
            self.broadcast_signal(OverseerSignal::ActiveLeaves(update))
                .await?;
        }
    ...
    fn handle_external_request(&mut self, request: ExternalRequest) {
        match request {
            ExternalRequest::WaitForActivation {
                hash,
                response_channel,
            } => {
