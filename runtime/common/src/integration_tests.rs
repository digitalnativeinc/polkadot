...
use crate::{
    ...
    slots,
    traits::{Auctioneer, Registrar as RegistrarT},
};
use frame_support::{
...
fn maybe_new_session(n: u32) {
    if n % BLOCKS_PER_SESSION == 0 {
        shared::Module::<Test>::set_session_index(shared::Module::<Test>::session_index() + 1);
...
fn basic_swap_works() {
    ...
    new_test_ext().execute_with(|| {
        ...
        // Paras should be onboarding
        assert_eq!(
            Paras::lifecycle(ParaId::from(2000)),
            Some(ParaLifecycle::Onboarding)
        );
        assert_eq!(
            Paras::lifecycle(ParaId::from(2001)),
            Some(ParaLifecycle::Onboarding)
        );
