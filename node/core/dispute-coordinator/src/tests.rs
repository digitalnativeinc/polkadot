...
use polkadot_node_subsystem::{jaeger, ActivatedLeaf, ActiveLeavesUpdate, LeafStatus};
...
where
    F: FnOnce(TestState, VirtualOverseer) -> BoxFuture<'static, ()>,
{
    ...
    test_harness(|test_state, mut virtual_overseer| {
        Box::pin(async move {
            ...
            let valid_vote = test_state
                .issue_statement_with_index(0, candidate_hash, session, true)
                .await;

            let invalid_vote = test_state
                .issue_statement_with_index(1, candidate_hash, session, false)
                .await;

            let invalid_vote_2 = test_state
                .issue_statement_with_index(2, candidate_hash, session, false)
                .await;
    ...
    test_harness(|test_state, mut virtual_overseer| {
        Box::pin(async move {
            ...
            let valid_vote_2 = test_state
                .issue_statement_with_index(1, candidate_hash, session, true)
                .await;
                ...
                virtual_overseer
                    .send(FromOverseer::Communication {
                        msg: DisputeCoordinatorMessage::DetermineUndisputedChain {
                            base_number: 10,
                            block_descriptions: vec![(block_hash_a, session, vec![candidate_hash])],
                            tx,
                        },
                    })
                    .await;
    ...
    test_harness(|test_state, mut virtual_overseer| {
        Box::pin(async move {
            ...
            for i in (0..supermajority_threshold - 1).map(|i| i + 2) {
                let vote = test_state
                    .issue_statement_with_index(i, candidate_hash, session, true)
                    .await;
