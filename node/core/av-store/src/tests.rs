...
async fn overseer_send(overseer: &mut VirtualOverseer, msg: AvailabilityStoreMessage) {
    ...
    overseer
        ...
        .await
        .expect(&format!(
            "{:?} is more than enough for sending messages.",
            TIMEOUT
        ));
}
...
async fn overseer_recv(overseer: &mut VirtualOverseer) -> AllMessages {
    let msg = overseer_recv_with_timeout(overseer, TIMEOUT)
        .await
        .expect(&format!(
            "{:?} is more than enough to receive messages",
            TIMEOUT
        ));
...
async fn overseer_signal(overseer: &mut VirtualOverseer, signal: OverseerSignal) {
    overseer
        ...
        .await
        .expect(&format!(
            "{:?} is more than enough for sending signals.",
            TIMEOUT
        ));
}
...
fn store_block_works() {
    ...
    test_harness(
        ...
        |mut virtual_overseer| async move {
            ...
            virtual_overseer
                .send(FromOverseer::Communication { msg: block_msg })
                .await;
            assert_eq!(rx.await.unwrap(), Ok(()));

            let pov = query_available_data(&mut virtual_overseer, candidate_hash)
                .await
                .unwrap();
...
fn stored_but_not_included_data_is_pruned() {
    ...
    test_harness(
        ...
        |mut virtual_overseer| async move {
            ...
            assert_eq!(
                query_available_data(&mut virtual_overseer, candidate_hash)
                    .await
                    .unwrap(),
                available_data,
            );
            ...
            test_state
                .clock
                .inc(test_state.pruning_config.keep_unavailable_for);
            test_state.wait_for_pruning().await;

            // The block was not included by this point so it should be pruned now.
            assert!(query_available_data(&mut virtual_overseer, candidate_hash)
                .await
                .is_none());
            virtual_overseer
        },
...
fn stored_data_kept_until_finalized() {
    ...
    test_harness(
        ...
        |mut virtual_overseer| async move {
            ...
            test_state
                .clock
                .inc(test_state.pruning_config.keep_unavailable_for * 10);
            test_state.wait_for_pruning().await;
            ...
            test_state
                .clock
                .inc(test_state.pruning_config.keep_finalized_for / 2);
            test_state.wait_for_pruning().await;
            ...
            test_state
                .clock
                .inc(test_state.pruning_config.keep_finalized_for);
            test_state.wait_for_pruning().await;
...
fn forkfullness_works() {
    ...
    test_harness(
        ...
        |mut virtual_overseer| async move {
            ...
            let validators: Vec<_> = (0..n_validators)
                .map(|_| Sr25519Keyring::Alice.public().into())
                .collect();
            ...
            assert_eq!(
                query_available_data(&mut virtual_overseer, candidate_1_hash)
                    .await
                    .unwrap(),
                available_data_1,
            );
            ...
            assert_eq!(
                query_available_data(&mut virtual_overseer, candidate_2_hash)
                    .await
                    .unwrap(),
                available_data_2,
            );
...
async fn query_available_data(
    ...
    virtual_overseer
        .send(FromOverseer::Communication { msg: query })
        .await;
...
async fn has_all_chunks(
    ...
    for i in 0..n_validators {
        if query_chunk(virtual_overseer, candidate_hash, ValidatorIndex(i))
            .await
            .is_some()
            != expect_present
        {
