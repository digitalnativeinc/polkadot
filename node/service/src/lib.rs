...
impl IdentifyVariant for Box<dyn ChainSpec> {
    ...
    fn is_rococo(&self) -> bool {
        self.id().starts_with("rococo") || self.id().starts_with("rco") || self.id().starts_with("barocco")
    }
    fn is_wococo(&self) -> bool {
    ...
    Executor: NativeExecutionDispatch + 'static,
{
...

    {
        ...
        config
            .network
            .extra_sets
            .extend(peer_sets_info(is_authority));
    }
    ...
    if !disable_beefy && (chain_spec.is_rococo() || chain_spec.is_wococo()) {
        ...
        let gadget =
            beefy_gadget::start_beefy_gadget::<_, beefy_primitives::ecdsa::AuthorityPair, _, _, _>(
                beefy_params,
            );
