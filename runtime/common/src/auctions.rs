...
impl<T: Config> Pallet<T> {
    ...
    pub fn handle_bid(
        ...
        // Bidding on latest auction.
            ...
            Error::<T>::NotCurrentAuction
        );
        // Assume it's actually an auction (this should never fail because of above).
        let (first_lease_period, early_end) =
            AuctionInfo::<T>::get().ok_or(Error::<T>::NotAuction)?;
        let late_end = early_end.saturating_add(T::EndingPeriod::get());
        ...
        if current_winning[range_index]
            .as_ref()
            .map_or(true, |last| amount > last.2)
        {
    ...
    fn calculate_winners(mut winning: WinningData<T>) -> WinnersData<T> {
        let winning_ranges = {
            ...
            best_winners_ending_at[SlotRange::LEASE_PERIODS_PER_SLOT - 1]
                .0
                .clone()
        };
...
mod tests {
        ...
        new_test_ext().execute_with(|| {
            ...
            for i in 1..6u64 {
                ...
                for j in 1..6 {
                    ...
                    assert_eq!(
                        Balances::free_balance(j),
                        if j == i { j * 9 } else { j * 10 }
                    );
                }
            }
    ...
    fn multiple_bids_work_post_ending() {
        new_test_ext().execute_with(|| {
            ...
            for i in 1..6u64 {
                assert_eq!(
                    ReservedAmounts::<Test>::get((i, ParaId::from(0))).unwrap(),
                    i
                );
            }
