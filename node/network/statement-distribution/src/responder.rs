...
pub async fn respond(
    ...
    loop {
        ...
        let req = IncomingRequest::new(peer, payload, pending_response);
...
fn report_peer(tx: oneshot::Sender<sc_network::config::OutgoingResponse>, rep: Rep) {
    if let Err(_) = tx.send(sc_network::config::OutgoingResponse {
        result: Err(()),
        reputation_changes: vec![rep.into_base_rep()],
        sent_feedback: None,
    }) {
        tracing::debug!(target: LOG_TARGET, "Reporting peer failed.");
    }
}
