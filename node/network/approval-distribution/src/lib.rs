...
impl State {
    async fn handle_network_msg(
        ...
        match event {
            ...
            NetworkBridgeEvent::PeerMessage(peer_id, msg) => {
                self.process_incoming_peer_message(ctx, metrics, peer_id, msg)
                    .await;
            }
        }
    }
    ...
    async fn handle_peer_view_change(
        ...
        tracing::trace!(target: LOG_TARGET, ?view, ?lucky, "Peer view change",);
        ...
        Self::unify_with_peer(ctx, metrics, &mut self.blocks, peer_id.clone(), view).await;
    }
    ...
    async fn import_and_circulate_assignment(
        ...
        if let Some(peer_id) = source.peer_id() {
            ...
            ctx.send_message(AllMessages::ApprovalVoting(
        ...
        match entry.candidates.get_mut(claimed_candidate_index as usize) {
            Some(candidate_entry) => {
                ...
                candidate_entry
                    ...
                    .or_insert_with(|| {
                        (
                            ApprovalState::Assigned(assignment.cert.clone()),
                            local_source,
                        )
                    });
            }
    ...
    async fn import_and_circulate_approval(
        ...
        if let Some(peer_id) = source.peer_id() {
            ...
            if !entry
                .knowledge
                .known_messages
                .contains(&assignment_fingerprint)
            {
                tracing::debug!(
                    target: LOG_TARGET,
                    ?peer_id,
        ...
        loop {
            ...
            match message {
                FromOverseer::Communication {
                    ...
                    state
                        .handle_network_msg(&mut ctx, &self.metrics, event)
                        .await;
                }
                FromOverseer::Communication {
                    ...
                    state
                        .handle_new_blocks(&mut ctx, &self.metrics, metas)
                        .await;
                }
                FromOverseer::Communication {
