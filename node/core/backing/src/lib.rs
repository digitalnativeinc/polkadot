...
async fn make_pov_available(
...

    {
        let _span = span
            .as_ref()
            .map(|s| s.child("erasure-coding").with_candidate(candidate_hash));
...

    {
        let _span = span
            .as_ref()
            .map(|s| s.child("store-data").with_candidate(candidate_hash));
...
impl CandidateBackingJob {
            ...
            ValidatedCandidateCommand::AttestNoPoV(candidate_hash) => {
                if let Some((attesting, span)) = self.fallbacks.get_mut(&candidate_hash) {
                    if let Some(index) = attesting.backing.pop() {
                        ...
                        self.kick_off_validation_work(sender, attesting, c_span)
                            .await?
                    }
                } else {
                    tracing::warn!(
    ...
    async fn sign_import_and_distribute_statement(
        ...
        if let Some(signed_statement) = self.sign_statement(statement).await {
            self.import_statement(sender, &signed_statement, root_span)
                .await?;
    ...
    async fn import_statement(
        ...
        let unbacked_span = if let Some(attested) = summary.as_ref().and_then(|s| {
            self.table
                .attested_candidate(&s.candidate, &self.table_context)
        }) {
    ...
    async fn process_msg(
        ...
        match msg {
            ...
            CandidateBackingMessage::Statement(_relay_parent, statement) => {
                ...
                match self
                    .maybe_validate_and_import(&root_span, sender, statement)
                    .await
                {
        ...
        if let Some(summary) = self.import_statement(sender, &statement, root_span).await? {
            ...
            let (attesting, span) = match statement.payload() {
                Statement::Seconded(receipt) => {
                    ...
                    self.fallbacks
                        .insert(summary.candidate, (attesting.clone(), span));
                    (attesting, child)
                }
            ...
            self.kick_off_validation_work(sender, attesting, span)
                .await?;
        }
        Ok(())
    }
    ...
    fn get_unbacked_statement_child(
        ...
        self.insert_or_get_unbacked_span(parent_span, hash, None)
            .map(|span| {
...
mod tests {
    ...
    fn make_erasure_root(test: &TestState, pov: PoV) -> Hash {
        ...
        let chunks =
            erasure_coding::obtain_chunks_v1(test.validators.len(), &available_data).unwrap();
    ...
    fn backing_second_works() {
        ...
        test_harness(
            ...
            |mut virtual_overseer| async move {
                ...
                let expected_head_data =
                    test_state.head_data.get(&test_state.chain_ids[0]).unwrap();
                ...
                virtual_overseer
                    .send(FromOverseer::Communication { msg: second })
                    .await;
