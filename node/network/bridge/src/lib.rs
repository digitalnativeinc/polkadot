...
use polkadot_subsystem::{
    messages::StatementDistributionMessage, ActivatedLeaf, ActiveLeavesUpdate, FromOverseer,
...
async fn update_our_view(
    ...
    let (validation_peers, collation_peers) = {
        ...
        match shared.local_view {
            Some(ref v) if v.check_heads_eq(&new_view) => return Ok(()),
...
fn dispatch_collation_event_to_all_unbounded(
    ...
    if let Some(msg) = event
        .focus()
        .ok()
        .map(CollatorProtocolMessage::NetworkBridgeUpdateV1)
    {
