...
use polkadot_primitives::v1::{
    ApprovalVote, BlockNumber, CandidateDescriptor, CandidateHash, CandidateIndex,
    CandidateReceipt, GroupIndex, Hash, PersistedValidationData, SessionIndex, SessionInfo,
    ValidationCode, ValidatorId, ValidatorIndex, ValidatorPair, ValidatorSignature,
};
...
impl Metrics {
    ...
    fn on_approval_error(&self) {
        if let Some(metrics) = &self.0 {
            metrics
                .approvals_produced_total
                .with_label_values(&["internal error"])
                .inc()
        }
    }
    ...
    fn time_db_transaction(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.time_db_transaction.start_timer())
    }

    fn time_recover_and_approve(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.time_recover_and_approve.start_timer())
    }
}
...
impl Wakeups {
    ...
    fn schedule(
        ...
        self.reverse_wakeups
            .insert((block_hash, candidate_hash), tick);
        self.wakeups
            .entry(tick)
            .or_default()
...
mod approval_db_v1_reader {
    use super::{
        approval_db, BlockEntry, CandidateEntry, CandidateHash, DBReader, DatabaseConfig, Hash,
        KeyValueDB, SubsystemError, SubsystemResult,
    };
    ...
    impl<T> ApprovalDBV1Reader<T> {
        pub(super) fn new(inner: T, config: DatabaseConfig) -> Self {
            ApprovalDBV1Reader { inner, config }
        }
    }

    impl<'a, T: 'a> DBReader for ApprovalDBV1Reader<T>
    where
        T: std::ops::Deref<Target = (dyn KeyValueDB + 'a)>,
    {
        fn load_block_entry(&self, block_hash: &Hash) -> SubsystemResult<Option<BlockEntry>> {
...
impl<T> State<T> {
    ...
    fn approval_status<'a, 'b>(
        ...
        let tranche_now = self
            .clock
            .tranche_now(self.slot_duration_millis, block_entry.slot());
        ...
        match action {
            ...
            Action::BecomeActive => {
                *mode = Mode::Active;
                ...
                let messages =
                    distribution_messages_for_activation(ApprovalDBV1Reader::new(db, db_config))?;

                ctx.send_messages(messages.into_iter().map(Into::into))
                    .await;
            }
            Action::Conclude => {
                conclude = true;
...
// finalized block.
fn cleanup_background_tasks(current_finalized_block: BlockNumber, tasks: &mut BackgroundTaskMap) {
...
async fn handle_approved_ancestor(
    ...
    let target_number = {
        ...
        ctx.send_message(ChainApiMessage::BlockNumber(target, tx).into())
            .await;
...
fn schedule_wakeup_action(
    ...
    let maybe_action = match required_tranches {
        ...
        RequiredTranches::Exact { next_no_show, .. } => {
            next_no_show.map(|tick| Action::ScheduleWakeup {
                block_hash,
                block_number,
                candidate_hash,
                tick,
...
fn check_and_import_assignment(
    ...
    actions.push(Action::WriteCandidateEntry(
        assigned_candidate_hash,
        candidate_entry,
    ));
...
fn check_and_import_approval<T>(
    ...
    let approval_payload =
        ApprovalVote(approved_candidate_hash).signing_payload(block_entry.session());
    ...
    let approval_sig_valid = approval
        .signature
        .verify(approval_payload.as_slice(), pubkey);
...
fn process_wakeup(
    ...
    let (mut actions, maybe_cert) = if should_trigger {
        ...
        let actions = vec![Action::WriteCandidateEntry(
            candidate_hash,
    ...
    if let Some((cert, val_index, tranche)) = maybe_cert {
        ...
        let index_in_candidate = block_entry
            .candidates()
            .iter()
...
async fn launch_approval(
    ...
    let background = async move {
        ...
        let _ = background_tx
            .send(BackgroundRequest::CandidateValidation(
        ...
        match val_rx.await {
            ...
            Ok(Ok(ValidationResult::Valid(_, _))) => {
                ...
                let _ = background_tx
                    .send(BackgroundRequest::ApprovalVote(ApprovalVoteRequest {
                        validator_index,
                        block_hash,
                        candidate_index,
                    }))
