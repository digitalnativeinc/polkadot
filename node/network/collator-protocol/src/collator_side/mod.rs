...
async fn distribute_collation(
    ...
    let validator_group: HashSet<_> = current_validators
        .validators
        .iter()
        .map(Clone::clone)
        .collect();
    ...
    state
        ...
        .insert(relay_parent, validator_group.into());
        ...
        CollationSeconded(relay_parent, statement) => {
            if !matches!(statement.unchecked_payload(), Statement::Seconded(_)) {
                ...
                let statement = runtime
                    .check_signature(ctx, relay_parent, statement)
                    .await?
