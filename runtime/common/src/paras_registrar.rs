...
impl<T: Config> Pallet<T> {
    ...
    fn do_reserve(
        ...
        id: ParaId,
            ...
            paras::Pallet::<T>::lifecycle(id).is_none(),
    ...
    fn validate_onboarding_data(
        ...
        parachain: bool,
            ...
            validation_code.0.len() <= T::MaxCodeSize::get() as usize,
            Error::<T>::CodeTooLarge
        );
        ensure!(
            genesis_head.0.len() <= T::MaxHeadSize::get() as usize,
...
mod tests {
            ...
            assert_noop!(
                Registrar::register(
                    ...
                    test_validation_code(
                        (<Test as super::Config>::MaxCodeSize::get() + 1) as usize
                    ),
                ),
    ...
    fn deregister_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            ...
            // Owner check
            assert_noop!(
                Registrar::deregister(Origin::signed(2), para_id,),
                BadOrigin
            );
            assert_ok!(Registrar::make_parachain(para_id));
            run_to_session(4);
    ...
    fn para_lock_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_noop!(
                Registrar::swap(Origin::signed(1), para_id, para_id + 2),
                BadOrigin
            );
        });
    }
