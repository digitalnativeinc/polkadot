...
fn decode_candidate_votes_key(key: &[u8]) -> Option<(SessionIndex, CandidateHash)> {
    ...
    CandidateHash::decode(&mut &key[(15 + 4)..])
        .ok()
        .map(|hash| (session, hash))
}
...
impl ActiveDisputes {
    ...
    pub(crate) fn contains(&self, session: SessionIndex, candidate_hash: CandidateHash) -> bool {
    ...
    pub(crate) fn insert(&mut self, session: SessionIndex, candidate_hash: CandidateHash) -> bool {
        let new_entry = (session, candidate_hash);

        let pos = self.disputed.iter().take_while(|&e| &new_entry < e).count();
    ...
    pub(crate) fn delete(&mut self, session: SessionIndex, candidate_hash: CandidateHash) -> bool {
        let new_entry = (session, candidate_hash);
...
impl Transaction {
    ...
    pub(crate) fn put_candidate_votes(
        ...
        self.write_candidate_votes
            .push((session, candidate_hash, votes))
    }
    ...
    match load_earliest_session(store, config)? {
        ...
        Some(prev_earliest) if new_earliest > prev_earliest => {
            ...
            // Clear active disputes metadata.
            {
                ...
                let prune_up_to = active_disputes
                    .disputed
                    .iter()
            ...
            // Clear all candidate data with session less than the new earliest kept.
            {
                ...
                store
                    .iter_with_prefix(config.col_data, CANDIDATE_VOTES_SUBKEY)
...
mod tests {
    ...
    fn db_transaction() {
...

        {
            ...
            tx.put_active_disputes(ActiveDisputes {
                disputed: vec![(0, CandidateHash(Hash::repeat_byte(0)))],
            });

            tx.put_active_disputes(ActiveDisputes {
                disputed: vec![(1, CandidateHash(Hash::repeat_byte(1)))],
            });
        ...
        {
            assert_eq!(load_earliest_session(&store, &config).unwrap().unwrap(), 1,);
            ...
            assert_eq!(
                load_candidate_votes(&store, &config, 1, &CandidateHash(Hash::repeat_byte(1)))
                    .unwrap()
                    .unwrap()
                    .candidate_receipt
                    .descriptor
    ...
    fn note_current_session_prunes_old() {
...

        {
            ...
            tx.put_candidate_votes(very_old, hash_a, blank_candidate_votes());

            tx.put_candidate_votes(slightly_old, hash_b, blank_candidate_votes());

            tx.put_candidate_votes(new_earliest_session, hash_c, blank_candidate_votes());

            tx.put_candidate_votes(very_recent, hash_d, blank_candidate_votes());
        ...
        assert!(load_candidate_votes(&store, &config, very_old, &hash_a)
            .unwrap()
            .is_none());
        assert!(load_candidate_votes(&store, &config, slightly_old, &hash_b)
            .unwrap()
            .is_none());
        assert!(
            load_candidate_votes(&store, &config, new_earliest_session, &hash_c)
                .unwrap()
                .is_some()
        );
        assert!(load_candidate_votes(&store, &config, very_recent, &hash_d)
            .unwrap()
            .is_some());
    }
}
