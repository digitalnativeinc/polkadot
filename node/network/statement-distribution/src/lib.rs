...
fn is_statement_large(statement: &SignedFullStatement) -> bool {
    match &statement.payload() {
        Statement::Seconded(committed) => {
            ...
            let threshold = PeerSet::Validation
                .get_info(IsAuthority::Yes)
...
async fn circulate_statement<'a>(
    ...
    let mut peers_to_send = util::choose_random_sqrt_subset(peers_to_send, MIN_GOSSIP_PEERS);
...
async fn launch_request(
    ...
    let available_peers = {
        ...
        m.insert(
            peer,
            vec![protocol_v1::StatementDistributionMessage::LargeStatement(
                meta,
            )],
        );
        m
...
async fn handle_incoming_message_and_circulate<'a>(
    ...
    if let Some((relay_parent, statement)) = handled_incoming {
        ...
        let _ = circulate_statement(peers, ctx, relay_parent, statement, Vec::new()).await;
    }
}
...
async fn handle_incoming_message<'a>(
    ...
    match active_head.note_statement(statement) {
        ...
        NotedStatement::Fresh(statement) => {
            ...
            let message = AllMessages::CandidateBacking(CandidateBackingMessage::Statement(
                relay_parent,
    ...
    async fn handle_responder_message(
        ...
        match message {
            ResponderMessage::GetData {
                ...
                tx.send(committed)
                    .map_err(|_| NonFatal::ResponderGetDataCanceled)?;
            }
        }
        Ok(())
    }
    ...
    async fn handle_requester_message(
        ...
        match message {
            ...
            RequesterMessage::SendRequest(req) => {
                ctx.send_message(AllMessages::NetworkBridge(
                    NetworkBridgeMessage::SendRequests(vec![req], IfDisconnected::ImmediateError),
                ))
                .await;
            }
        ...
        match message {
            FromOverseer::Signal(OverseerSignal::ActiveLeaves(ActiveLeavesUpdate {
                ...
                for activated in activated {
                    ...
                    let info = runtime
                        .get_session_info_by_index(ctx, relay_parent, session_index)
                        .await?;
