...
async fn process_block_activated(
    ...
    let block_number = {
        let (tx, rx) = oneshot::channel();
        ctx.send_message(ChainApiMessage::BlockNumber(activated, tx).into())
            .await;
    ...
    let block_header = {
        ...
        ctx.send_message(ChainApiMessage::BlockHeader(activated, tx).into())
            .await;
    ...
    let n_validators = {
        let (tx, rx) = oneshot::channel();
        ctx.send_message(
            RuntimeApiMessage::Request(block_header.parent_hash, RuntimeApiRequest::Validators(tx))
                .into(),
        )
        .await;
...
async fn process_block_finalized(
    ...
    loop {
        ...
        let batch_finalized_hash = if batch_num == finalized_number {
            ...
            let (tx, rx) = oneshot::channel();
            ctx.send_message(ChainApiMessage::FinalizedBlockHash(batch_num, tx).into())
                .await;
...
fn process_message(
    ...
    match msg {
        AvailabilityStoreMessage::QueryAvailableData(candidate, tx) => {
            let _ = tx.send(load_available_data(
                &subsystem.db,
                &subsystem.config,
                &candidate,
            )?);
...
impl Metrics {
    ...
    fn time_process_block_finalized(
        ...
        self.0
            .as_ref()
            .map(|metrics| metrics.process_block_finalized.start_timer())
    }
    ...
    fn time_block_activated(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.block_activated.start_timer())
    }
    ...
    fn time_process_message(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.process_message.start_timer())
    }
    ...
    fn time_store_available_data(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.store_available_data.start_timer())
    }
    ...
    fn time_store_chunk(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.store_chunk.start_timer())
    }
    ...
    fn time_get_chunk(&self) -> Option<metrics::prometheus::prometheus::HistogramTimer> {
        self.0
            .as_ref()
            .map(|metrics| metrics.get_chunk.start_timer())
    }
}
