...
use frame_support::traits::{Currency, EnsureOrigin, ExistenceRequirement, Get, VestingSchedule};
use frame_system::pallet_prelude::*;
pub use pallet::*;
use parity_scale_codec::{Decode, Encode};
use sp_core::sr25519;
use sp_runtime::traits::{CheckedAdd, Saturating, Verify, Zero};
use sp_runtime::{AnySignature, DispatchError, DispatchResult, Permill, RuntimeDebug};
use sp_std::prelude::*;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn payout(origin: OriginFor<T>, who: T::AccountId) -> DispatchResult {
            ...
            let payment_account = ensure_signed(origin)?;
                ...
                payment_account == PaymentAccount::<T>::get(),
...
mod tests {
    ...
    fn signature_verification_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(Purchase::verify_signature(
                &alice_ed25519(),
                &alice_signature_ed25519()
            ));
    ...
    fn payout_handles_basic_errors() {
        new_test_ext().execute_with(|| {
            // Wrong Origin
            assert_noop!(
                Purchase::payout(Origin::signed(alice()), alice(),),
                BadOrigin
            );
