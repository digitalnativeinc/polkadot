...
use polkadot_node_subsystem::{
    messages::{AllMessages, ProvisionerMessage},
    SubsystemError,
};
...
impl ParachainsInherentDataProvider {
    ...
    pub async fn create<C: HeaderBackend<Block>>(
        ...
        let pid = async {
            ...
            let (sender, receiver) = futures::channel::oneshot::channel();
            overseer
                .send_msg(AllMessages::Provisioner(
            ...
            receiver
                .await
                .map_err(|_| Error::ClosedChannelAwaitingInherentData)
        };
...
pub enum Error {
    ...
    #[error(
        "Timeout: provisioner did not return inherent data after {:?}",
        PROVISIONER_TIMEOUT
    )]
    Timeout,
