...
use polkadot_node_network_protocol::{request_response::request::Requests, ObservedRole};
...
use polkadot_subsystem::{jaeger, ActiveLeavesUpdate, FromOverseer, LeafStatus, OverseerSignal};
...
fn do_not_send_view_update_when_only_finalized_block_changed() {
    test_harness(done_syncing_oracle(), |test_harness| async move {
        ...
        virtual_overseer
            .send(FromOverseer::Signal(OverseerSignal::BlockFinalized(
                Hash::random(),
                5,
            )))
            .await;
...
fn peer_messages_sent_via_overseer() {
    test_harness(done_syncing_oracle(), |test_harness| async move {
        ...
        network_handle
            .disconnect_peer(peer.clone(), PeerSet::Validation)
            .await;
    ...
    for msg in AllMessages::dispatch_iter(NetworkBridgeEvent::PeerDisconnected(PeerId::random())) {
        match msg {
            ...
            AllMessages::DisputeParticipation(_) => {
                unreachable!("Not interetsed in network events")
