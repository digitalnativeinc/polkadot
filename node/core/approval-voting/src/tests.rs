...
impl MockClockInner {
    ...
    fn wakeup_all(&mut self, up_to: Tick) {
        ...
        let drain_up_to = self
            .wakeups
            .binary_search_by_key(&(up_to + 1), |w| w.0)
            .unwrap_or_else(|i| i);
    ...
    fn register_wakeup(&mut self, tick: Tick, pre_emptive: bool) -> oneshot::Receiver<()> {
        ...
        let pos = self
            .wakeups
            .binary_search_by_key(&tick, |w| w.0)
            .unwrap_or_else(|i| i);
...
fn single_session_state(index: SessionIndex, info: SessionInfo) -> State<TestStore> {
    State {
...
fn sign_approval(
    ...
    key.sign(&ApprovalVote(candidate_hash).signing_payload(session_index))
        .into()
}
...
fn some_state(config: StateConfig) -> State<TestStore> {
    ...
    add_block(&mut state.db, block_hash, session_index, slot);
...
fn add_block(db: &mut TestStore, block_hash: Hash, session: SessionIndex, slot: Slot) {
...
fn rejects_bad_assignment() {
    ...
    assert!(res
        .1
        .iter()
        .any(|action| matches!(action, Action::WriteCandidateEntry(..))));
    ...
    assert_eq!(
        res.0,
        AssignmentCheckResult::Bad(AssignmentCheckError::UnknownBlock(unknown_hash))
    );
...
fn rejects_approval_before_assignment() {
    ...
    assert_eq!(
        res,
        ApprovalCheckResult::Bad(ApprovalCheckError::NoAssignment(ValidatorIndex(0)))
    );
...
fn rejects_approval_if_no_block_entry() {
    ...
    assert_eq!(
        res,
        ApprovalCheckResult::Bad(ApprovalCheckError::UnknownBlock(block_hash))
    );
    assert!(actions.is_empty());
}
...
fn second_approval_import_only_schedules_wakeups() {
    ...
    let (actions, res) =
        check_and_import_approval(&state, &Metrics(None), vote.clone(), |r| r).unwrap();
