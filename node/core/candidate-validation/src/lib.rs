...
async fn validate_candidate_exhaustive(
    ...
    let result = match result {
        ...
        Err(ValidationError::InvalidCandidate(WasmInvalidCandidate::WorkerReportedError(e))) => Ok(
...
mod tests {
    ...
    fn candidate_validation_bad_return_is_invalid() {
        ...
        assert_matches!(
            v,
            ValidationResult::Invalid(InvalidCandidate::ExecutionError(_))
        );
    }
    ...
    fn candidate_validation_code_mismatch_is_invalid() {
        ...
        assert_matches!(
            v,
            ValidationResult::Invalid(InvalidCandidate::CodeHashMismatch)
        );
    }
    ...
    fn compressed_code_works() {
        ...
        let validation_code =
            sp_maybe_compressed_blob::compress(&raw_code, VALIDATION_CODE_BOMB_LIMIT)
                .map(ValidationCode)
                .unwrap();
