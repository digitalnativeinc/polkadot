...
pub(crate) fn canonicalize(
    ...
    {
        let mut frontier: Vec<_> = pruned_branches
            .into_iter()
            .map(|h| (canon_number + 1, h))
            .collect();
...
fn load_decode<D: Decode>(store: &dyn KeyValueDB, col_data: u32, key: &[u8]) -> Result<Option<D>> {
...
fn load_stored_blocks(store: &dyn KeyValueDB, config: &Config) -> Result<Option<StoredBlockRange>> {
