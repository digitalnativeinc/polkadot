...
use frame_support::traits::{
    tokens::currency::Currency as CurrencyT, Get, OnUnbalanced as OnUnbalancedT,
};
use frame_support::weights::{GetDispatchInfo, Weight, WeightToFeePolynomial};
...
impl<T: Get<Weight>, C: Decode + GetDispatchInfo> WeightBounds<C> for FixedWeightBounds<T, C> {
    fn shallow(message: &mut Xcm<C>) -> Result<Weight, ()> {
        Ok(match message {
            ...
            Xcm::RelayedFrom {
                ref mut message, ..
            } => T::get() + Self::shallow(message.as_mut())?,
...
impl<T: Get<(MultiLocation, u128)>, R: TakeRevenue> Drop for FixedRateOfConcreteFungible<T, R> {
    fn drop(&mut self) {
        let revenue = MultiAsset::ConcreteFungible {
            amount: self.1,
            id: T::get().0,
        };
