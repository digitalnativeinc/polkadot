...
use futures::{executor, select, Future, FutureExt, StreamExt};
...
fn task_can_be_canceled() {
    ...
    assert!(
        running_task.poll(&mut ctx) == Poll::Ready(()),
        "Task is immediately finished"
    );
}
    ...
    async fn handle_message(&self, msg: AllMessages) -> bool {
        match msg {
            ...
            AllMessages::AvailabilityStore(AvailabilityStoreMessage::StoreChunk {
                ...
                tx.send(Ok(()))
                    .expect("Answering fetching task should work");
                return true;
            }
            _ => {
