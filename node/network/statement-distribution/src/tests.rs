...
use polkadot_node_network_protocol::{request_response::Recipient, view, ObservedRole};
...
fn active_head_accepts_only_2_seconded_per_validator() {
    ...
    assert!(head_data
        .check_useful_or_unknown(&a_seconded_val_0.clone().into())
        .is_ok());
    ...
    .expect("should be signed");
    assert!(head_data
        .check_useful_or_unknown(&statement.clone().into())
        .is_ok());
...
fn per_peer_relay_parent_knowledge_send() {
    ...
    assert_eq!(
        knowledge.send(&(CompactStatement::Seconded(hash_a), ValidatorIndex(0))),
        true
    );
    assert_eq!(
        knowledge.send(&(CompactStatement::Seconded(hash_a), ValidatorIndex(1))),
        false
    );
    assert!(knowledge.is_known_candidate(&hash_a));
    ...
    assert_eq!(
        knowledge.send(&(CompactStatement::Valid(hash_a), ValidatorIndex(0))),
        false
    );
    assert!(knowledge.is_known_candidate(&hash_a));
...
fn circulated_statement_goes_to_all_peers_with_view() {
    ...
    executor::block_on(async move {
        ...
        let needs_dependents =
            circulate_statement(&mut peer_data, &mut ctx, hash_b, statement, Vec::new()).await;
...
fn share_prioritizes_backing_group() {
    ...
    let dummy_pairs: Vec<_> = std::iter::repeat_with(|| Pair::generate().0)
        .take(dummy_count)
        .collect();
    let dummy_peers: Vec<_> = std::iter::repeat_with(|| PeerId::random())
        .take(dummy_count)
        .collect();
