...
fn collator_peer_id(
    ...
    peer_data.iter().find_map(|(peer, data)| {
        data.collator_id()
            .filter(|c| c == &collator_id)
            .map(|_| peer.clone())
    })
}

async fn disconnect_peer(ctx: &mut impl SubsystemContext, peer_id: PeerId) {
    ctx.send_message(NetworkBridgeMessage::DisconnectPeer(peer_id, PeerSet::Collation).into())
        .await
}
    ...
    Context: SubsystemContext<Message = CollatorProtocolMessage>,
{
    ...
    match msg {
        Declare(collator_id, para_id, signature) => {
            ...
            if !signature.verify(
                &*protocol_v1::declare_signature_payload(&origin),
                &collator_id,
            ) {
        ...
        AdvertiseCollation(relay_parent) => {
            ...
            match peer_data.insert_advertisement(relay_parent, &state.view) {
                Ok((id, para_id)) => {
                    ...
                    let (tx, rx) = oneshot::channel::<(CandidateReceipt, PoV)>();
                    ...
                    let future = rx.map(|r| ((id, pending_collation), r));
...
async fn remove_relay_parent(state: &mut State, relay_parent: Hash) -> Result<()> {
    state
        .requested_collations
        .retain(|k, _| k.relay_parent != relay_parent);
...
async fn handle_our_view_change(
    ...
    state
        .active_paras
        .assign_incoming(ctx.sender(), keystore, added)
        .await;
    ...
    Context: SubsystemContext<Message = CollatorProtocolMessage>,
{
    ...
    match bridge_message {
        PeerConnected(peer_id, _role, _) => {
            ...
            state
                .metrics
                .note_collator_peer_count(state.peer_data.len());
        }
        ...
        Invalid(parent, candidate_receipt) => {
            if match state.pending_candidates.get(&parent) {
                Some(collation_event)
                    if Some(candidate_receipt.commitments_hash)
                        == collation_event.1.commitments_hash =>
                {
                    true
                }
                _ => false,
    ...
    Context: SubsystemContext<Message = CollatorProtocolMessage>,
{
    ...
    loop {
        ...
        state
            .requested_collations
            .retain(|k, _| retained_requested.contains(k));
    }
    Ok(())
}
    ...
    Context: SubsystemContext,
{
    ...
    if let Poll::Ready(response) = futures::poll!(&mut per_req.from_collator) {
        ...
        per_req
            .span
            .as_mut()
            .map(|s| s.add_string_tag("success", success));
        true
    } else {
        false
    }
