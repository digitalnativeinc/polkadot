...
impl ParaLifecycle {
    ...
    pub fn is_offboarding(&self) -> bool {
        matches!(
            self,
            ParaLifecycle::OffboardingParathread | ParaLifecycle::OffboardingParachain
        )
    }
...
impl<N: Ord + Copy + PartialEq> ParaPastCodeMeta<N> {
    ...
    fn note_replacement(&mut self, expected_at: N, activated_at: N) {
        self.upgrade_times.push(ReplacementTimes {
            expected_at,
            activated_at,
        })
    }
    ...
    fn code_at(&self, para_at: N) -> Option<UseCodeAt<N>> {
        ...
        if let Some(replaced_after_pos) = replaced_after_pos {
            ...
            if was_pruned {
                ...
                Some(UseCodeAt::ReplacedAt(
                    self.upgrade_times[replaced_after_pos].expected_at,
                ))
            }
        } else {
    ...
    fn prune_up_to(&'_ mut self, max: N) -> impl Iterator<Item = N> + '_ {
        let to_prune = self
            .upgrade_times
            .iter()
            .take_while(|t| t.activated_at <= max)
            .count();
...
fn build<T: Config>(config: &GenesisConfig<T>) {
    let mut parachains: Vec<_> = config
        .paras
        .iter()
...
impl<T: Config> Module<T> {
    ...
    fn note_past_code(
        ...
        <Self as Store>::PastCodePruning::mutate(|pruning| {
            let insert_idx = pruning
                .binary_search_by_key(&at, |&(_, b)| b)
    ...
    pub(crate) fn schedule_para_initialize(id: ParaId, genesis: ParaGenesisArgs) -> DispatchResult {
        ...
        // Make sure parachain isn't already in our system.
            ...
            Self::can_schedule_para_initialize(&id, &genesis),
            Error::<T>::CannotOnboard
        );
    ...
    pub(crate) fn schedule_parathread_upgrade(id: ParaId) -> DispatchResult {
        ...
        let lifecycle = ParaLifecycles::get(&id).ok_or(Error::<T>::NotRegistered)?;
            ...
            lifecycle == ParaLifecycle::Parathread,
...
mod tests {
    ...
    use crate::mock::{new_test_ext, MockGenesisConfig, Paras, Shared, System};
    ...
    fn check_code_is_stored(validation_code: &ValidationCode) {
        ...
        assert!(<Paras as Store>::CodeByHash::contains_key(
            validation_code.hash()
        ));
    }

    fn check_code_is_not_stored(validation_code: &ValidationCode) {
        assert!(!<Paras as Store>::CodeByHashRefs::contains_key(
            validation_code.hash()
        ));
        assert!(!<Paras as Store>::CodeByHash::contains_key(
            validation_code.hash()
    ...
    fn para_past_code_pruning_works_correctly() {
        ...
        assert_eq!(
            past_code.prune_up_to(60).collect::<Vec<_>>(),
            vec![30, 40, 50]
        );
        assert_eq!(
            past_code,
    ...
    fn para_past_code_pruning_in_initialize() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            assert_eq!(
                Paras::past_code_meta(&id).most_recent_change(),
                Some(at_block)
            );
            check_code_is_stored(&validation_code);
    ...
    fn note_past_code_sets_up_pruning_correctly() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            assert_eq!(
                <Paras as Store>::PastCodePruning::get(),
                vec![(id_a, 12), (id_b, 23)]
            );
            assert_eq!(
    ...
    fn code_upgrade_applied_after_delay() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            let expected_at = {
                ...
                assert!(Paras::past_code_meta(&para_id)
                    .most_recent_change()
                    .is_none());
                assert_eq!(
                ...
                assert_eq!(
                    <Paras as Store>::FutureCodeHash::get(&para_id),
                    Some(new_code.hash())
                );
                assert_eq!(Paras::current_code(&para_id), Some(original_code.clone()));
                check_code_is_stored(&original_code);
    ...
    fn full_parachain_cleanup_storage() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            // Just scheduling cleanup shouldn't change anything.
            {
                ...
                assert_eq!(
                    <Paras as Store>::Heads::get(&para_id),
    ...
    fn code_hash_at_with_intermediate() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            assert_eq!(
                fetch_validation_code_at(para_id, 1, None),
                Some(old_code.clone())
            );
            assert_eq!(
                fetch_validation_code_at(para_id, 10, None),
                Some(old_code.clone())
            );
            assert_eq!(
                fetch_validation_code_at(para_id, 100, None),
                Some(old_code.clone())
            );

            // intermediate before upgrade meant to be applied, falls back on current.
            assert_eq!(
                fetch_validation_code_at(para_id, 9, Some(8)),
                Some(old_code.clone())
            );
            assert_eq!(
                fetch_validation_code_at(para_id, 10, Some(9)),
                Some(old_code.clone())
            );
            assert_eq!(
                fetch_validation_code_at(para_id, 11, Some(9)),
                Some(old_code.clone())
            );

            // intermediate at or after upgrade applied
            assert_eq!(
                fetch_validation_code_at(para_id, 11, Some(10)),
                Some(new_code.clone())
            );
            assert_eq!(
                fetch_validation_code_at(para_id, 100, Some(11)),
                Some(new_code.clone())
            );
    ...
    fn code_hash_at_returns_up_to_end_of_code_retention_period() {
        ...
        new_test_ext(genesis_config).execute_with(|| {
            ...
            assert_eq!(
                fetch_validation_code_at(para_id, 2, None),
                Some(old_code.clone())
            );
            assert_eq!(
                fetch_validation_code_at(para_id, 3, None),
                Some(old_code.clone())
            );
            assert_eq!(
                fetch_validation_code_at(para_id, 9, None),
                Some(old_code.clone())
            );
            assert_eq!(
                fetch_validation_code_at(para_id, 10, None),
            ...
            assert_eq!(
                fetch_validation_code_at(para_id, 11, None),
                Some(new_code.clone())
            );
        });
    }
