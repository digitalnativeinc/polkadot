...
use futures::{channel::mpsc, channel::oneshot, FutureExt, SinkExt, StreamExt};
...
use super::mock::{make_ferdie_keystore, make_session_info, OccupiedCoreBuilder};
...
fn to_incoming_req(
    ...
    match outgoing {
        Requests::ChunkFetching(OutgoingRequest {
            ...
            let (tx, rx): (
                oneshot::Sender<netconfig::OutgoingResponse>,
                oneshot::Receiver<_>,
            ) = oneshot::channel();
            executor.spawn(
                "Message forwarding",
